...
public class McpServerIntegrationTests : IClassFixture<AppTestServerFixture>
{
    ...
    [Fact]
    public async Task McpEndpoint_StatelessToolsCall_WithArgumentsAndMeta_Succeeds()
    {
        // 2026-07-28 MCP specification: parameterized tool call with _meta context
        var payload = new
        {
            jsonrpc = "2.0",
            id = 4,
    ...
    [Fact]
    public async Task McpEndpoint_MultipleSequentialStatelessCalls_NoSessionStateCoupling()
    {
        // 2026-07-28 MCP specification: each request is completely independent and stateless
        for (int i = 1; i <= 3; i++)
        {
            var payload = new
            {
